...
const findItem = (items, key, withIndex) => {
  let result = null;
...
$(() => {
  $('#employees').dxTreeList({
    dataSource: {
      ...
      insert(values) {
        const parentItem = findItem(employees, values.Parent_ID);
      ...
      update(key, values) {
        const item = findItem(employees, key);
        if (item) {
          Object.assign(item, values);
        }
      },
      remove(key) {
        const itemWithIndex = findItem(employees, key, true);
        if (itemWithIndex) {
